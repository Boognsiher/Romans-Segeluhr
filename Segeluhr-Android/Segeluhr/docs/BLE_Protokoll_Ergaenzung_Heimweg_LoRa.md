...
### Paketformat (11 Byte, little-endian)
...
} HomeLoRaPacket; // = 11 Bytes
```

Bewusst kompakt gehalten (kleine Pakete = bessere Reichweite/Robustheit
bei LoRa, kürzere Sendezeit).
...
void onLoRaPacketReceived(uint8_t* data, size_t len) {
  ...
  // z.B. "Segler auf dem Heimweg — ETA 42 min" + kleine Karte/Koordinaten
  Serial.printf("Heimweg-Update: ETA %u min, Position %.5f, %.5f%s\n",
                pkt.etaMinutes, lat, lon,
