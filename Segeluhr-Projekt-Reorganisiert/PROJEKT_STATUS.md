...
| Galaxy-Watch-App (Wear OS) | Samsung Galaxy Watch5 Pro LTE | 🔧 | — | 07.08.2026 neu angelegt (`Segeluhr-Android/Segeluhr/wear/`, `docs/Erweiterung_GalaxyWatch_WearOS.md`) — Alternative zur **T-Watch Ultra** (die einzige Boots-Uhr mit BLE-Central-Rolle zum Handy; die S3 ist reine Land-Uhr für Begleiter, ausschliesslich per LoRa versorgt, siehe Zeile oben), **bewusst ohne LoRa** (übernimmt nur die BLE-Client-Rolle der Ultra fürs Boot, keine Boot→Land-Fernverbindung — wer die S3-Land-Funktion braucht, bleibt auf die bestehende Ultra+LoRa-Kombi angewiesen). Erster Schritt: reiner BLE-Verbindungstest (Scan/Connect/Subscribe GPS+Battery+Wind, Compose-for-Wear-Screen mit Rohwerten), keine echten Segeluhr-Screens, kein CMD_*/Haptik/Reconnect. **⚠️ NICHT kompiliert**, noch nicht auf Hardware getestet — insbesondere ob die eigene BLE-GATT-Verbindung parallel zur normalen Wear-Companion-Kopplung funktioniert. Details/offene Punkte siehe Doku. |
...
Segeluhr-Android/Segeluhr/     Android-App (Kotlin/Compose)
  app/                         Handy-App (GATT-Server)
  wear/                        🔧 Galaxy-Watch-App (Wear OS, GATT-Client, ohne LoRa)
