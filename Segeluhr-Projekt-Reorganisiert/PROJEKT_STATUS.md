...
| Windschätzung (robust/kontinuierlich) | – (reine Software, `WindEngine`) | 🔧 | — | **22.09.2026 Konzept-Phase** (Speed-Dip-Hypothese für wind-unabhängige Wende/Halse-Erkennung gegen die 15./16.08.-Diagnose-Logs geprüft, Skript `docs/diagnose-logs/tack_gybe_speed_check.py` — Ergebnis uneinheitlich, zusätzlich fragwürdige Ground Truth wegen des erst 17.08. gefixten Windshift-Bugs in genau diesen Logs, daher NICHT umgesetzt). **22.09.2026 implementiert** (vor dem Segelwochenende, Roman-Wunsch): `windDir` kommt jetzt aus einem Ringpuffer gewichteter Samples (`WindEngine.windHistory`) statt einer fortlaufend verschobenen Einzelzahl — jede erkannte Wende/Halse beim normalen Segeln liefert automatisch ein weiteres Sample (Bisektor-Trick löst die 180°-Mehrdeutigkeit ohne Wende/Halse-Klassifikation), der explizite Kalibrierlauf bleibt nur noch für den allerersten Wert der Session nötig. Neue "Vertrauen"-Anzeige im Wind-Tab + `wind_sample_count`-Spalte im Diagnose-Log. **Keine Watch-/Firmware-Änderung nötig** (BLE-Wind-Characteristic unverändert). **22.09.2026 Nachtrag (Roman-Frage zu Regatta-Ausweich-/Bojenmanövern):** neuer Filter `WindEngine.isPlausibleRacingLeg()` verwirft während Competition/Trainings-Racemode Legs, deren Kurswinkel weder nahe am Wende- noch am Vorwind-Winkel liegt (Halbwindkurse/Ausweichen/Dial-up) — verhindert, dass solche Legs die Windschätzung verfälschen oder eine falsche "Wind-Shift"-Meldung mitten im Ausweichmanöver auslösen. Dabei nebenbei einen Bug im CSV-Reimport (`DiagnosticsLogImporter.kt`) gefunden+gefixt: feste Spalten-Indizes wären durch die neue `wind_sample_count`-Spalte verschoben gewesen, jetzt auf Namens-Auflösung umgestellt. Details: `docs/Erweiterung_Windschaetzung_Robust.md`. **Nicht kompiliert** — diese Umgebung hat kein Android-SDK/keinen Netzwerkzugriff auf die Google/Maven-Repos, nur manuell gegengelesen. **Vor dem Wochenende in Android Studio kompilieren + kurz antesten.** |
...
## 17.08.2026: Windshift-Fix kompiliert + automatische Tages-Auswertung

**Windshift-Plausibilitätsfilter vom 16.08.** (`Constants.WIND_SHIFT_MAX_PLAUSIBLE_DEG`,
`WindEngine.tickContinuous()`) war bis heute nur geschrieben, nie kompiliert —
`compileDebugKotlin` + `assembleDebug` jetzt beide BUILD SUCCESSFUL, kein
Compile-Fehler. Funktionale Verifikation weiterhin offen (nächster Törn).

**Neu: automatische Tages-/Wettfahrt-Auswertung** (Roman-Wunsch, "wie wenn
ich ein Log auswerten lasse", später erweitert um PDF+Karte+Verlauf-Tab,
siehe `docs/Erweiterung_Tages_Auswertung.md`) — Pendant zur bisher manuellen
CSV-Auswertung der Diagnose-Logs (15./16.08., siehe oben), jetzt direkt in
der App. Neuer `SessionReport` (Dauer, Distanz, Max-/Ø-Speed, Anzahl Wenden/
Halsen + Ø-Winkel, Wind-Shifts Header/Lift, Windkalibrierungen, gelernter
Wende-/Vorwind-Winkel, Uhr-Verbindungsanteil, Route), gebaut von
`logic/SessionSummaryEngine.kt` (zeichnet einen vollständigen GPS-Track auf,
Bericht für ein BELIEBIGES Zeitfenster) zusammen mit zeitgestempelten
Event-Listen in `WindEngine.kt` (`sessionManeuvers`/`sessionWindShifts`/
`sessionCalibrations` — Wende-/Halsen-Erkennung nutzt bewusst denselben
`tickContinuous()`-Bug-Wechsel-Moment, den die Header/Lift-Erkennung
ohnehin schon verfolgt, keinen neuen Post-hoc-Algorithmus). Erscheint
automatisch als Dialog bei App-Stopp (**DAY**, ganzer Tag, überlebt Stopp/
Start-Zyklen) UND bei "Wettfahrt beenden" (**RACE**, eigenes Zeitfenster ab
Countdown-Start — startet ein neuer Countdown, ohne dass die vorige
Wettfahrt sauber beendet wurde, wird sie still ohne Dialog gespeichert,
Roman-Vorgabe). Beide Arten werden in neuer Room-Tabelle `sessions`
persistiert (DB-Version 1→2, `fallbackToDestructiveMigration`) und im neuen
**"Verlauf"-Tab** (`SessionHistoryScreen.kt`) wieder aufrufbar — Route live
auf osmdroid-Karte (`RouteMapView.kt`). **"Als PDF teilen"**
(`SessionPdfExporter.kt`, `android.graphics.pdf.PdfDocument`, kein neuer
Dependency) baut ein Ein-Seiten-PDF mit Statistik + echtem OSM-Karten-
ausschnitt der Route (Roman: "wie bei Strava") — bewusst akzeptiertes
Robustheitsrisiko: Kartenkacheln kommen von einer nie ans Fenster
angehängten Offscreen-`MapView` mit fester Wartezeit statt einem echten
"fertig geladen"-Signal, siehe Doku. **Kompiliert** (beide Gradle-Tasks
grün), **App startet auf Hardware ohne Absturz** (inkl. destruktiver DB-
Migration) — PDF-Kartenkacheln und Wettfahrt-Session-Grenzen (inkl.
"still speichern bei erneutem Countdown-Start") noch nicht gezielt
getestet, dafür braucht es einen echten Törn/Start.

## 17.08.2026 Abend: Diagnose-Log-Import + vier Hardware-Bugs gefixt

Nach dem ersten Hardware-Test der Tages-/Wettfahrt-Auswertung (siehe oben)
in mehreren Runden nachgebessert, alle Fixes **auf Hardware verifiziert**:

- **Diagnose-Log-Import** (Roman-Wunsch): neuer Button im Verlauf-Tab
  importiert eine zuvor geteilte Diagnose-CSV rückwirkend als Session
  (`DiagnosticsLogImporter.kt`, spielt die Zeilen durch dieselbe
  `WindEngine`-Logik wie live). Dabei Bugfix gefunden: importierte Sessions
  zeigten "heute, 0min" statt der echten Log-Zeit — `SessionSummaryEngine`/
  `WindEngine` stempelten Events mit `System.currentTimeMillis()` statt dem
  GPS-Fix-Zeitstempel, für Live-Betrieb folgenlos, beim schnellen CSV-
  Replay aber fatal. Gefixt (`fix.timestampMs` an allen vier Stellen).
- **PDF-Karte zeigte nur Platzhalter-Raster**: `CacheManager.downloadAreaAsync()`
  (osmdroids Bulk-Vorlade-API) lehnt die freie MAPNIK/OSM-Quelle strikt ab
  (`TileSourcePolicyException`) — **und crashte dabei die App**, weil die
  Exception auf einem rohen Hintergrund-Thread flog, ausserhalb jeder
  Coroutine-Fehlerbehandlung. Zurückgebaut auf Einzel-Kachel-Laden; dabei
  einen zweiten, eigentlichen Bug gefunden: `mapView.draw()` wurde nur EIN
  Mal NACH der Wartezeit aufgerufen, obwohl osmdroid Kachel-Downloads erst
  BEIM Zeichnen anstösst — die Wartezeit lief also komplett ins Leere.
  Gefixt: zweimal zeichnen (vor + nach der Wartezeit), Download-Threads
  2→8 erhöht. Roman bestätigt: "Kacheln sind vorhanden".
- **Karte überlappte die Auswertung im Verlauf-Detail-Screen**: zwei Bugs
  in Serie — `fillMaxSize()` statt `weight(1f)` auf dem Stats-Block
  (Compose-Höhenberechnungs-Klassiker) UND zusätzlich clippte die native
  osmdroid-`MapView` ihr Zeichnen nicht auf die Compose-Box. Beide gefixt.
- **PDF-Export wirkte wie "Knopf tut nichts"**: Export dauert durch die
  Kachel-Wartezeit ~8s ohne jede Rückmeldung — neuer blockierender
  Ladedialog ("PDF wird erstellt…") an beiden "Als PDF teilen"-Stellen.

Details/Volltext aller vier Bugfixes: `docs/Erweiterung_Tages_Auswertung.md`
