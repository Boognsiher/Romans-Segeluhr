...
| Windschätzung (robust/kontinuierlich) | – (reine Software, `WindEngine`) | 📋 | — | **22.09.2026 Konzept-Phase**: Roman-Wunsch, dass Windschätzung stabiler wird, ohne Pflicht-Kalibrierlauf grundsätzlich mitläuft und sich aus der Historie zusammensetzt statt nur eine Einzelzahl fortzuschreiben — Ziel: vor dem Ablegen nur noch "Start" drücken, Rest (inkl. Amwind/Downwind-Erkennung) automatisch. Speed-Dip-Hypothese für wind-unabhängige Wende/Halse-Erkennung gegen die 15./16.08.-Diagnose-Logs geprüft (Skript `docs/diagnose-logs/tack_gybe_speed_check.py`) — Ergebnis uneinheitlich, zusätzlich fragwürdige Ground Truth wegen des erst 17.08. gefixten Windshift-Bugs in genau diesen Logs. Details/offene Punkte: `docs/Erweiterung_Windschaetzung_Robust.md`. Noch keine Zeile Code in `WindEngine` geändert, braucht sauber markierte Test-Manöver vom nächsten Törn. |
